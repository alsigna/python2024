...
Вызов функции в коде происходит через её имя и передачу аргументов (если требуется):
...
get_interface_name("wrong-string")
# >>> ''
...
Ключевое слово `return` означает завершение работы функции, удаляется стек функции (область памяти, которая была создана под ёе выполнение), удаляются все переменные, которые были созданы внутри функции, пользователю возвращается только результат (то, что написано после `return`).
...
Локальные переменные могут маскировать глобальные, т.е. можно использовать одно и то же имя, при этом внутри функции будет использовано значение локальной переменной, а за пределами функции будет использовано значение глобальной переменной.
...
Ключевое слово `nonlocal` позволяет выходить из локальной области видимости на верхний уровень (но не на глобальный). Это ключевое слово актуально когда используются вложенные функции.
...
При инициализации и вызове функции аргументы (и параметры) можно разделить на несколько вариантов:
...
> при использовании изменяемых объектов в качестве значений по умолчанию, следует помнить, что эти объекты создаются при инициализации функции, и при повторных вызовах будут использованы те же данные, что и при предыдущем вызове.
...
Позиционные - при вызове функции учитывается позиция аргументов и передаваемые значения записываются в переменные согласно их порядку:
...
Если используется распаковка, то список/кортеж распаковывается в позиционные, а словарь - в именованные переменные, ключи используются как имена для аргументов, поэтому должны совпадать с именами параметров функции.
...
Можно отключить возможность использовать позиционные параметры, для этого используется символ `*` и следующая запись:
...
В python есть встроенные функции `globals()` и `locals()` который позволяют посмотреть содержимое соответствующей области видимости (глобальную и локальную).

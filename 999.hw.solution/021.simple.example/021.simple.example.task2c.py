...
def apply_rule(func, rules):
    def inner(value):
        method = getattr(value, func.__name__)
...
full_if_name = apply_rule(str.startswith, if_name_format_rules)
strip_domain = apply_rule(str.endswith, domain_format_rules)

for line in lldp_output.splitlines():
    if_name, peer, *_ = line.split()
    if_name = full_if_name(if_name)
    lldp_dict[if_name] = peer

for line in description_output.splitlines():
    if_name, *_, peer = line.split()
    peer = strip_domain(peer)
